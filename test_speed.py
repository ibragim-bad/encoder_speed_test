...
def run_model(texts, model, tokenizer: AutoTokenizer, bs, only_tokenize, cut, max_length):
    embs = []
    texts = [t[:cut] for t in texts]
    for bt in batch(texts, n=bs):
        batch_dict = tokenizer.batch_encode_plus(bt, max_length=max_length, truncation=True,
            return_tensors="pt",
        )
...
def main(
    ...
    for model_name in config["models"]:
        ...
        loaded(**tokenizer.batch_encode_plus([text], max_length=512, truncation=True,
            return_tensors="pt",
        ))
        ...
        for param_combo in params:
            ...
            for _ in range(config["num_runs"]):
                st = time.monotonic()
                embs = run_model(docs, loaded, tokenizer, param_combo["batch_size"], only_tokenize=False, cut=param_combo["clip_str"], max_length=param_combo["seq_len"])
            ...
            print(param_combo)
            print(f"{times.mean():.3f} per sample: {times.mean()/len(docs):.3f} ms, std_prc of runs: {times.std()/times.mean():.3f}")
            outputs.append(
                {
                    "model": model_name, 
